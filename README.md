...
It is important to pass the custom field ID by calling `set_sql_for_fieldid` immediately after instantiation.
This method takes the custom field ID instead of the shortname to maintain high performance.

```php
$newfilter->set_sql_for_fieldid($customfieldid);
```

If you need to select custom fields from a table other than `customfield_field` (for example, `user_custom_field`), you need to pass your preferred SQL into the filter using the `set_sql` function.

```php
$newfilter->set_sql($sql, $columnname);
```

The provided `$sql` must follow the structure of a subquery `SELECT` statement containing the keywords `SELECT`, `FROM`, and `WHERE`, with table names enclosed in `{}` and a `:where` placeholder.

The argument `$columnname` is the name of the column that holds the value inside the desired table.
For example, if we want to create a filter on a custom profile field named `supervisor`, we can create a custom field filter as follows:
