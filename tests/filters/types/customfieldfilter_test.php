...
final class customfieldfilter_test extends advanced_testcase {
    ...
    public function test_if_customfiledfilter_filters_the_records_sample1(): void {
        ...
        $customfieldfilter->set_sql('category IN ( SELECT id FROM {course_categories} WHERE :where)');
        // The final query generated by this filter will be similar to 'category IN ( SELECT id FROM {course_categories} WHERE name name LIKE '%category2%')'.
        $customfieldfilter->set_subquery_column('name');
    ...
    public function test_if_customfiledfilter_filters_the_records_sample2(): void {
        ...
        ";
        $customfieldfilter->set_sql($subsql);
        $customfieldfilter->set_subquery_column('uid.data');
        $table->add_filter($customfieldfilter);
        $table->outhtml(10000, true);
